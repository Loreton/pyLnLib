...
class CalibreMetadataReader:
    """
    Lettore di metadati Calibre con accesso lazy.
    All'avvio carica solo gli indici (ID e autori).
    I libri vengono caricati singolarmente su richiesta.
    """
    ...
    def __init__(self, library_path: str):
        ...
        # Campi standard (inizializzati dopo aver verificato il DB)
    ...
    def _init_field_queries(self) -> None:
        """Inizializza le query per i campi standard in modo robusto"""
        self.field_queries = {
            'id': 'b.id',
            'title': 'b.title',
            'sort': 'b.sort',
    ...
    def _get_rating_query(self) -> str:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                # Verifica se la tabella ratings esiste
                cursor = conn.execute(
                ...
                if 'name' in columns:
                    ...
                    # Se non trova colonne, usa 'id' come fallback
                    col = 'id'
    ...
    def _load_indices(self) -> None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            self.ids: list[int] = [row[0] for row in cursor.fetchall()]
    ...
    def get_book(self, book_id: int, fields: list[str] | None = None) -> dict[str, object] | None:
        ...
        if book_id in self._cache:
            self.logger.debug(f"📖 ID {book_id} in cache")
            return self._cache[book_id]

        self.logger.debug(f"📖 ID {book_id} caricamento...")
    ...
    def _load_book(self, book_id: int, fields: list[str] | None = None) -> dict[str, object] | None:
        ...
        if fields is None:
            ...
            # 'rating' escluso per evitare problemi
    ...
    def get_books_batch(self, book_ids: list[int], fields: list[str] | None = None) -> dict[int, dict[str, object]]:
        ...
        # Recupera quelli in cache prima
