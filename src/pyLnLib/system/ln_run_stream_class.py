...
class lnRunStream_Class:
    def __init__(self, logger):
        ...
        self.RCLONE_CHANGE_PATTERN = re.compile(
            r"^(\s*\*\s+)"
        )  ###. riga che inizia con " * "
        # ^ = inizio riga
    ...
    def processConsoleLine(self, process, line: str):
        ...
        line_lower = (
            line.lower() if self.ignore_case else line
        )  ###. per ascoltare ignore_case
    ...
    def prepareWords(self, process_strings: dict):
        ...
        _unnecessary_starting_with = process_strings.pop(
            "unnecessary_starting_with", []
        )
        # Config per evidenziare le righe rsync che indicano un cambiamento
        self.rsync_change_color = process_strings.pop(
            "rsync", None
        )  # None oppure {"change_color": [start, end]}
        self.rclone_change_color = process_strings.pop(
            "rclone", None
        )  # None oppure {"change_color": [start, end]}

        # Normalizza solo se necessario
        if self.ignore_case:
            self.unnecessary_starting_with = [
                s.lower() for s in _unnecessary_starting_with
            ]
...
if __name__ == "__main__":
    ...
    cmd = [
        "/usr/bin/rsync",
        "--dry-run"
        "--exclude-from=/tmp/lnSync/rsync/myself/filesystem_exclude_patterns.txt",
        "--archive",
        "--verbose",
        "--update",
        "--partial",
        ...
        "--compress",
        "--timeout=15",
        '"/home/loreto/.config/FreeFileSync"',
        '"/home/loreto/filu/lnENV/config/appls Saved/IdeaPadSlim3/FreeFileSync"',
    ]
