...
class EpubManager:
    ...
    def parse_calibre_metadata(self) -> dict:
        ...
        def _get_calibre_attribute(meta_elem: etree.Element) -> tuple(str, str):
            name = meta.attrib.get("name", "") # name of column
            value = None
            ...
            content = meta.attrib.get("content", "") # content and struct of column
            try:
                d_content=json.loads(content) if content else {}
                d_content = lnDict(d_content) if isinstance(d_content, dict) else d_content
            except json.JSONDecodeError:
                d_content = None  # non era un dictionary/json
            # else:
            #     d_content = json.loads(content) if d_content else {}
            #     d_content = lnDict(d_content) if isinstance(d_content, dict) else d_content

            if d_content and isinstance(d_content, dict):
                self.logger.debug(f"meta.attrib.name:     {name}")
                self.logger.debug(f"meta.attrib.content:  {d_content}")
                value = d_content.get("#value#")
        ...
        for meta in metadata_elem.findall(".//opf:meta", namespaces=NAMESPACES):
            ...
            # field, value = _get_calibre_attribute(meta)
            # if field in calibre_custom_column:
                # calibre_data[field] = value
                # self.logger.info(f"{name}: {value}")

            '''
            self.logger.info(f"metadata name:        {type(name)} {name}")
            self.logger.info(f"metadata attribute:   {type(meta.attrib)} {meta.attrib}")
            self.logger.info(f"metadata text:        {type(meta.text)} {meta.text}")

            # self.logger.info(f"meta.attrib.name:     {type(meta.text)} {meta.attrib.get('name')}")
            # self.logger.info(f"meta.attrib.content:  {type(meta.text)} {meta.attrib.get('content')}")
            attr_name = meta.attrib.get("name", "")
            attr_content = meta.attrib.get("content", "")

            d_attr_content = None
            try:
                d_attr_content=json.loads(attr_content) if attr_content else {}
            except json.JSONDecodeError:
                pass
                # d_attr_content = attr_content
            else:
                d_attr_content=json.loads(attr_content) if attr_content else {}
                d_attr_content = lnDict(d_attr_content) if isinstance(d_attr_content, dict) else d_attr_content
            # self.logger.info(f"meta.attrib.content:  {d_content}")

            self.logger.info(f"meta.attrib.name:     {attr_name}")
            # self.logger.info(f"meta.attrib.content:  {d_attr_content}")

            if d_attr_content and isinstance(d_attr_content, dict):
                self.logger.info(f"meta.attrib.content:  {d_attr_content}")
                # for k, v in d_attr_content.items():
            ...
            if name and name.startswith("calibre:user_metadata:"):
                ...
                field = name.split(":", 2)[2] if ":" in name else name
                # if not field in calibre_custom_column and not field in calibre_fields: 
                    # continue
                ...
                '''
                if meta.text:
                    self.logger.info(f"\tmeta.text: {meta.text}")
                    try:
                        import json
                        value = json.loads(meta.text)
                        # Estrai il valore effettivo
                        if isinstance(value, dict) and "#value#" in value:
                            calibre_data[f"user_{field}"] = value["#value#"]
                        else:
                            calibre_data[f"user_{field}"] = meta.text
                    except:
                        calibre_data[f"user_{field}"] = meta.text

                elif meta.attrib.get("content"):
                    self.logger.info(f"\tmeta.attrib.content: {meta.attrib.get('content')}")
                    try:
                        import json
                        value = json.loads(meta.attrib.get("content"))
                        # Estrai il valore effettivo
                        if isinstance(value, dict) and "#value#" in value:
                            calibre_data[f"user_{field}"] = value["#value#"]
                        else:
                            calibre_data[f"user_{field}"] = meta.attrib.get("content")
                    except:
                        calibre_data[f"user_{field}"] = meta.attrib.get("content")
                '''
            ...
            # Cerca calibre:user_metadata (JSON completo)
                ...
                try:
                    ...
                    pass
        print(lnDict(calibre_data))
        # breakpoint()
        return calibre_data



    # def parse_metadata(self, opf_path: Path) -> EpubMetadata:
    #     """Parsa i metadati dal file OPF."""

    #     return self._parse_metadata(self._opf_path)
...
def test_epub_modify_metadata(book: EpubManager):
...
if __name__ == "__main__":
    ...
    file_list = scan_directory("/home/loreto/Downloads/test_epubs", "*.epub")
    ...
    for epub_path in file_list:
        ...
        try:
            ...
            # for key, value in calibre_data.items():
                # book.set_custom_metadata(f'calibre_{key}', value)

            # test_epub_modify_metadata(book=book)

            # 5. Salva in nuovo file
            output_file = book.path.parent / f"test_{book.path.name}"
            book.save(output_file)
            print(f"\n💾 Salvataggio completato: {output_file}")

            # 6. Esporta come testo
            txt_file = book.path.parent / f"{book.path.stem}_estratto.txt"
            if book.to_text(txt_file, replace=True):
                print(f"📄 Esportato come testo: {txt_file}")
